...
rows = [
    dict(fc=BLUE_L, ec=BLUE, tag="1 · Deterministic math",
         body="Deck-out countdown, prize-trade race,\nexact outs by hypergeometric count",
         tool="Race calculator\n(closed-form, no search)"),
    dict(fc=AQUA_L, ec=AQUA, tag="2 · Near-perfect information",
         body="Endgame: both decks are thin enough\nthat hidden information barely matters",
         tool="Let the search actually\nplay it out (endgame PUCT)"),
...
for i, r in enumerate(rows):
    y = top - i * (row_h + gap)
    box(ax, (0.1, y), 3.1, row_h, r["tag"], "#ffffff", r["ec"], tcolor=INK, fs=9.7, weight="bold", radius=0.08)
    box(ax, (3.5, y), 3.3, row_h, r["body"], r["fc"], r["ec"], fs=8.8)
    box(ax, (7.1, y), 2.8, row_h, r["tool"], "#ffffff", r["ec"], fs=9.0, weight="bold")
...
ax.text(0.1, footer_y,
        ...
        "model is allowed to operate. Two rejected upgrades (a gradient-boosted evaluator, a\n"
        "neural value net) both targeted layer 3 and both failed controlled A/B tests — evidence\n"
        "the bottleneck was never leaf-evaluation quality. The one confirmed win, endgame PUCT,\n"
        "targeted layer 2 instead: restructuring search where information is most complete.",
...
box(ax, (2.6, 1.15), 4.8, 1.35,
    "Outcome: the opponent runs out of resources and deck\nbefore Great Tusk / Crustle run out of ways to survive",
    "#ffffff", INK2, fs=9.6, weight="bold", radius=0.1)
