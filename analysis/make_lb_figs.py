...
ax.set_xticks(x[::3]); ax.set_xticklabels([d[5:] for d in days][::3], rotation=45, fontsize=8)
gap=[i for i,d in enumerate(days) if d=='2026-08-21']
if gap: ax.axvline(gap[0]-0.5, color='k', lw=1, ls=':'); ax.text(gap[0]-0.4, 92, 'submission deadline (Aug 16); post-deadline episodes →', fontsize=7.5)
h1,l1=ax.get_legend_handles_labels(); h2,l2=ax2.get_legend_handles_labels(); ax.legend(h1+h2,l1+l2, loc='lower left', fontsize=7, ncol=3, framealpha=0.9)
ax.set_title(f'Great Tusk lineage on the Kaggle ladder, Jul 19 – Aug 31: {len(LO):,} games across {len(set(r["sid"] for r in LO))} submissions. Daily win rate stays near 50% while the pool rotates.', fontsize=9.5)
...
fig,ax=plt.subplots(figsize=(9,4.6),dpi=160); x=np.arange(len(famsB)); w=0.38
for off,(rs,lab,c) in enumerate(((pre,'before Aug 17 (earlier builds)','#9bb7d4'),(post,'Aug 17 onward (final build)','#1f4e79'))):
    ...
    for i,(a,_,k) in enumerate(v): ax.text(x[i]+(off-0.5)*w, a+3.5, f'n={k}', ha='center', fontsize=6.5, color='#333')
ax.axhline(50, color='#999', lw=0.8, ls='--'); ax.set_ylim(0,90); ax.set_ylabel('ladder win rate (%) with 95% CI')
ax.set_xticks(x); ax.set_xticklabels(famsB, fontsize=8.5); ax.legend(fontsize=8)
ax.set_title('Matchup profile on the ladder: strong into Lucario / Archaludon, even into Alakazam, Grimmsnarl closed from 46% to 52%', fontsize=9.5)
...
for d,sc,lo,ref in fin: ax.annotate(f'final {sc}', (dt.datetime.strptime(d,'%Y-%m-%d'), sc), xytext=(-58,6), textcoords='offset points', fontsize=8, color='#d1495b')
ax.axhline(799.3, color='#d1495b', lw=0.8, ls='--')
los=[sc for _,sc,lo,_ in pts if lo]
ax.set_title(f'Converged rating of every submission (blue = Great Tusk lineage, {len(los)} builds, median {np.median(los):.0f}); the final entries sit at the top of the band', fontsize=9.5)
